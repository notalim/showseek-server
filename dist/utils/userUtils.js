 ...
 */
export const addMediaToUserWatched = async (userId, mediaId, title, rating, watchedOn) => {
    const userRef = db.collection("users").doc(userId);
    try {
 ...
 */
export const generateUserWeeklyRecap = async (userId) => {
    const user = await getUserById(userId) || {};
    console.log(`Generating recap for user: ${userId}, lastWatched: ${JSON.stringify(user.lastWatched)}`);
    if (user && user.lastWatched && user.lastWatched.length > 0) {
        const fullMedia = await Promise.all(user.lastWatched.map(async (mediaId) => {
 ...
 */
export const generateAllUserWeeklyRecaps = async () => {
